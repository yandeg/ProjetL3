...
public class Ecran0Controller {
    ...
    public static void loadCSVData () {
        ...
        try {
            InputStream inputStream = Ecran0Controller.class.getResourceAsStream("/files/"+nomDossier+"/elements.csv");
            if (inputStream == null) {
                throw new FileNotFoundException("File not found: " + "/files/user/element.csv");
            }
    ...
    public void initialize1() {
        ...
        public static void loadCSVData1 () {
            String line;

            try {
                InputStream inputStream = Ecran0Controller.class.getResourceAsStream("/files/"+nomDossier+"/Prix.csv");
                if (inputStream == null) {
                    throw new FileNotFoundException("File not found: " + "/files/user/Chaines.csv");
                }
                BufferedReader file = new BufferedReader(new InputStreamReader(inputStream));
         ...
         * @throws IOException Si une erreur d'entrée/sortie se produit lors de la lecture du fichier CSV.
         */
        public static void loadCSVData2 () {
            String line;

            try {
                InputStream inputStream = Ecran0Controller.class.getResourceAsStream("/files/"+nomDossier+"/Chaines.csv");
                if (inputStream == null) {
                    throw new FileNotFoundException("File not found: " + "/files/user/Chaines.csv");
                }
                BufferedReader file = new BufferedReader(new InputStreamReader(inputStream));
         ...
         */
        private static HashMap<String, Double> parseHashMap (String input){
